...
- Claim per process with `--author grok-bot/sales-pipeline` or `SECOND_BRAIN_IDENTITY=grok-bot/sales-pipeline`
- Do **not** use a single shared `knowledge/.identity.json` for a fleet.
- Chat prefix (optional): `Grok Bot: Pipeline Sales`
...
**Required:** type ownership. This pack may write: Lead, Opportunity, Stage, Objection, Proposal, Forecast. Refuse everything else.
